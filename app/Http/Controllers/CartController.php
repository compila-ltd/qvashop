...
class CartController extends Controller
{
        ...
        if (auth()->user() != null) {
            ...
            if($negotiable_transportation != null){
                return array(
                    ...
                    'modal_view' => view('frontend.partials.cantAddToCart')->render(),
    ...
    public function removeFromCart(Request $request)
    {
        ...
        if($request->id == 'all'){
            $user = auth()->user();
            if ($user != null) {
                $negotiable_transportations = \App\Models\NegotiableTransportation::where('user_id', $user->id)
                ->where('status', 1)
                ->get();
                
                if (!$negotiable_transportations->isEmpty()) {
                    foreach ($negotiable_transportations as $negotiable_transportation) {
                        $negotiable_transportation->update(['status' => 0]);
                    }
                }
