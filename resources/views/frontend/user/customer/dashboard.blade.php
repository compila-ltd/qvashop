...
@section('panel_content')
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0 h6">{{ translate('Dashboard') }}</h5>
        </div>
        <div class="card-body">
            <div class="row gutters-10">
                ...
                <div class="col-md-4">
                    <div class="bg-grad-2 text-white rounded-lg mb-4 overflow-hidden">
                        <div class="px-3 pt-3">
                            @php
                                $user_id = Auth::user()->id;
                                $wishlist = \App\Models\Wishlist::where('user_id', $user_id)->get();
                            @endphp
                            @if(count($wishlist) > 1)
                            <div class="h3 fw-700">
                                {{ count($wishlist) }} {{ translate('Product(s)') }}
                            </div>
                            @else
                            <div class="h3 fw-700">
                                {{ count($wishlist) }} {{ translate('Product') }}
                            </div>
                            @endif
                ...
                <div class="col-md-4">
                    <div class="bg-grad-3 text-white rounded-lg mb-4 overflow-hidden">
                        <div class="px-3 pt-3">
                            @php
                                $orders = \App\Models\Order::where('user_id', Auth::user()->id)->where('payment_status', 'paid')->get();
                                $total = 0;
                            ...
                            @if($total > 1)
                                <div class="h3 fw-700">
                                    {{ $total }} {{ translate('Product(s)') }}
                                </div>
                            @else
                                <div class="h3 fw-700">
                                    {{ $total }} {{ translate('Product') }}
                                </div>
                            @endif
