<form class="form-horizontal" action="{{ route('commissions.pay_to_seller') }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="modal-header">
        ...
        @else
            <h5 class="modal-title h6">{{ translate('Details')}}</h5>
        @endif
    ...
    <div class="modal-body">
        <table class="table table-striped table-bordered">
            <tbody>
                <tr>
                    @if(($user->shop->admin_to_pay >= 0)&&($seller_withdraw_request->status == "0"))
        ...
        @if ($user->shop->admin_to_pay > 0)
            ...
            <div class="form-group row">
                ...
                @else
                    <label class="col-sm-3 col-from-label" for="amount">{{ translate('Paid')}}</label>
                @endif
                <div class="col-sm-9">
                    @if ($seller_withdraw_request->amount > $user->shop->admin_to_pay)
                        <input type="number" readonly="readonly" lang="en" min="0" step="0.01" name="amount" id="amount" value="{{ $user->shop->admin_to_pay }}" class="form-control" required>
                    @else
                        <input type="number" readonly="readonly" lang="en" min="0" step="0.01" name="amount" id="amount" value="{{ $seller_withdraw_request->amount }}" class="form-control" required>
                    @endif
                </div>
            </div>
